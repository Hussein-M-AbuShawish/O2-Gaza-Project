...
const RAMADAN_PACKAGE = {
  name: "بكج المقبلات الرمضاني المجاني",
  items: ["تمر", "مياه", "مشروب خروب", "كنافة عربية", "شوربة فريكة"],
  image: "/images/our-images/1.jpg",
  desc: "ينزل مجاناً مع كل وجبة يتم طلبها"
};
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mb-12 relative">
             <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-20 pointer-events-none">
                <Moon size={120} className="text-amber-500" />
             </div>
             <h1 className="text-3xl md:text-5xl font-bold text-amber-500 mb-4 z-10 relative">
               حجز فرع {branchName}
             </h1>
          ...
          <motion.div 
            ...
            className="max-w-4xl mx-auto mb-16 bg-[#1a1a1a] rounded-3xl overflow-hidden border-2 border-amber-600/30 shadow-2xl shadow-amber-600/10"
          >
            <div className="md:flex">
              <div className="md:w-1/3 h-64 md:h-auto bg-amber-900/20 relative">
                <img src={RAMADAN_PACKAGE.image} alt="بكج رمضان" className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-[#1a1a1a] to-transparent"></div>
              </div>
              <div className="md:w-2/3 p-8 flex flex-col justify-center">
                <h2 className="text-2xl font-bold text-amber-400 mb-4">{RAMADAN_PACKAGE.name}</h2>
                ...
                <p className="text-sm text-amber-200 italic font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            {RESERVATION_ITEMS.map((item) => (
              <motion.div 
                key={item.id}
                whileHover={{ y: -10 }}
                className="bg-[#1a1a1a] rounded-2xl overflow-hidden border border-white/5 hover:border-amber-500/30 transition-all shadow-xl"
              >
      ...
      {cart.length > 0 && !showCheckout && (
        ...
        >
          <button 
            ...
            className="w-full bg-amber-600 text-white py-4 rounded-2xl font-bold shadow-2xl flex items-center justify-between px-8 hover:bg-amber-500 transition-colors"
          >
        ...
        {showCheckout && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-2xl bg-[#141414] rounded-3xl overflow-hidden border border-white/10 shadow-2xl max-h-[90vh] flex flex-col"
            >
              {/* Checkout Header */}
                  ...
                  {cart.map((i) => (
                    <div key={i.item.id} className="bg-[#1f1f1f] p-4 rounded-2xl flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-amber-500 mt-1">+ {RAMADAN_PACKAGE.name}</p>
                      </div>
              ...
              <div className="p-6 border-t border-white/10 bg-[#1a1a1a]">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <span className="text-2xl font-bold text-amber-500">{totalPrice} شيكل</span>
                </div>
                <button 
